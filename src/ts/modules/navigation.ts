...
const navDesktop = function() {
  const navPrimary = document.querySelector('.nav-primary') as HTMLElement;
  ...
  // Define globals
  const menuItems = navPrimary.querySelectorAll('.menu-item') as NodeListOf<HTMLElement>;
...
const navClick = function() {
  ...
  const dropdownMenuItems = document.querySelectorAll('.menu-item-has-children') as NodeListOf<HTMLElement>;
  ...
  // Define focusable elements on sub-menu (.menu-item a, .dropdown button)
  const focusableElementsforDropdown = document.querySelectorAll('.menu-item a, .dropdown button, .button-nav') as NodeListOf<HTMLElement>;
...
const navMobile = function() {
  ...
  function navToggle(e: Event) {
    const target = e.target as HTMLElement;

    // If clicked with mouse or enter key
    if (e.type === 'click' || (e as KeyboardEvent).keyCode === 13) {
      // Activate nav
      ...
      if (navToggleElement?.getAttribute('aria-expanded') === 'false') {
      ...
      // Toggle aria-label
      if (navToggleElement?.getAttribute('aria-label') === air_light_screenReaderText.expand_toggle) {
        navToggleElement.setAttribute('aria-label', air_light_screenReaderText.collapse_toggle);
      } else {
        navToggleElement?.setAttribute('aria-label', air_light_screenReaderText.expand_toggle);
      }
      ...
      focusableElements.forEach(function(menuItem: HTMLElement) {
  ...
  const dropdownToggles = document.querySelectorAll('.dropdown-toggle') as NodeListOf<HTMLElement>;

  // Loop through dropdown-toggles
  dropdownToggles.forEach(function(dropdownToggle: HTMLElement) {
...
const navSticky = function() {
  ...
  function initStickyNav() {
    ...
    const headerHeight = parseFloat(getComputedStyle(siteHeader).height);
